		...
		struct load_weight *lw)
{
	 ...
	 */
	if (likely(weight > (1UL << SCHED_LOAD_RESOLUTION/*0*/)))
		tmp = (u64)delta_exec * scale_load_down(weight)/*weight*/;
	else
		tmp = (u64)delta_exec;
	...
	if (!lw->inv_weight) {
		...
		if (BITS_PER_LONG > 32 && unlikely(w >= WMULT_CONST/*~0UL*/))
			lw->inv_weight = 1;
		else if (unlikely(!w))
...
// 2016-10-22
// min_vruntime(vruntime, se->vruntime)
...
// 2016-10-22
// 2016-11-05
// min_vruntime < vruntime : min_vruntime값을 재갱신
static void update_min_vruntime(struct cfs_rq *cfs_rq)
{
	...
	// min_vruntime 값보다 vruntime이 클 때 값을 갱신
...
{
	if (unlikely(se->load.weight != NICE_0_LOAD/*0x400, 1KB*/))
 ...
 */
// 업데이트 대상 : curr의 멤버 필드, cfs_rq->vmin_runtime
// 2016-10-22
	      ...
	      unsigned long delta_exec)
{
	...
	schedstat_set(curr->statistics.exec_max,
		      max((u64)delta_exec, curr->statistics.exec_max)); // NO OP.

	curr->sum_exec_runtime += delta_exec;
	schedstat_add(cfs_rq, exec_clock, delta_exec); // No OP.
...
static void update_curr(struct cfs_rq *cfs_rq)
{
	...
	// CONFIG_FAIR_GROUP_SCHED 미 정의로 entity_is_task() 함수는
	// 항상 TRUE를 리턴
	if (entity_is_task(curr)) {
		struct task_struct *curtask = task_of(curr);

		trace_sched_stat_runtime(curtask, delta_exec, curr->vruntime);
		cpuacct_charge(curtask, delta_exec); // NO OP.
		account_group_exec_runtime(curtask, delta_exec);
	}

	account_cfs_rq_runtime(cfs_rq, delta_exec); // No OP.
}
...
{
	// cfs->rq의 load.weight 값 감소
	update_load_sub(&cfs_rq->load, se->load.weight);
	if (!parent_entity(se))
		// 상위 런큐의 load.weight값 변경
		update_load_sub(&rq_of(cfs_rq)->load, se->load.weight);
	if (entity_is_task(se))
		// se->group_node의 연결 제거 및 노드 초기화
...
// 2016-10-22
// 2016-11-05
// decay_load(cfs_rq->blocked_load_avg, decays)
static __always_inline u64 decay_load(u64 val, u64 n)
{
	 ...
	 */
	// LOAD_AVG_PERIOD 주기를 맞추기 위해 local_n을 계산
	// local_n이 6 일 때: 6 = 6 % 32
	//           33 일 때: 1 = 6 % 32
...
static u32 __compute_runnable_contrib(u64 n)
{
	...
	if (likely(n <= LOAD_AVG_PERIOD/*32*/))
		return runnable_avg_yN_sum[n];
	else if (unlikely(n >= LOAD_AVG_MAX_N/*345*/))
		return LOAD_AVG_MAX/*47742*/;

	// LOAD_AVG_PERIOD/*32*/ < n < LOAD_AVG_MAX_N/*345*/
	/* Compute \Sum k^n combining precomputed values for k^i, \Sum k^j */
	do {
		contrib /= 2; /* y^LOAD_AVG_PERIOD = 1/2 */
		contrib += runnable_avg_yN_sum[LOAD_AVG_PERIOD];

		// LOAD_AVG_PERIOD 주기로 하기 위해 뺌
		n -= LOAD_AVG_PERIOD;
...
static inline u64 __synchronize_entity_decay(struct sched_entity *se)
{
	...
	// decay_count == 0 -> 동기화가 되었다는 것을 알려줌.
...
static inline void __update_task_entity_contrib(struct sched_entity *se)
{
	...
	// 가중치(weight)가 높고, runnable_avg_period가 작으면
	// 계산된 contrib 값은 크게 나오며
...
static long __update_entity_load_avg_contrib(struct sched_entity *se)
{
	...
	// 2016-10-22 CONFIG_FAIR_GROUP_SCHED 미 정의로 entity_is_task() 함수는
	// 항상 true를 리턴
	if (entity_is_task(se)) {
...
static void update_cfs_rq_blocked_load(struct cfs_rq *cfs_rq, int force_update)
{
	...
	// blocked_load_avg를 감소시켜야 하는 경우
	// (주석으로 보면 이후 깨어날 태스크인 경우 감소시킬 load값이 존재하는 경우로 추정)
	...
	// 처음 감쇄한 시간으로부터 시간이 지난 경우
	if (decays) {
		// blocked_load_avg값을 감쇄, 감쇄 횟수 및 이전 감쇄 시간 업데이트 
	...
	__update_cfs_rq_tg_load_contrib(cfs_rq, force_update);// No OP.
}
						  ...
						  int sleep)
{
	...
	// 2016-11-05 시작
	// 위의 두 함수에서는 avg.load_avg_contrib 값을 변경하지 않았다.
	// 주석 대로 미리 계산된 cfs_rq의 자식 load_average값을 제거하는 것으로 추정
	cfs_rq->runnable_load_avg -= se->avg.load_avg_contrib;
	if (sleep) {
...
enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	 ...
	 */
	// ENQUEUE_WAKEUP 미설정 또는 ENQUEUE_WAKING 설정인 경우
...
{
	// 검사할 sched_entity가 cfs_rq의 last, next, skip과 같다면 
	// 해당 포인터 값을 null로 초기화한다.
	if (cfs_rq->last == se)
		__clear_buddies_last(se);
...
// cfs_rq의 vruntime, min_vruntime 변경, se 정보를 cfs_rq에서 제거
static void
dequeue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	// 2016-11-05
	// se가 cfs_rq의 next, skip, last와 같다면 포인터를 null로 초기화한다
	clear_buddies(cfs_rq, se);

	if (se != cfs_rq->curr)
		// 2016-11-05
		// se가 현재 수행 중이지 않은 경우 rb tree에서 삭제
		__dequeue_entity(cfs_rq, se);
	se->on_rq = 0;
	// 2016-11-05
	// cfs_rq, rq의 load.weight값 감소, se의 group_node연결을  제거 및 노드 초기화
	account_entity_dequeue(cfs_rq, se);
	// 2016-11-05 식사 전
	/*
	...
	// se가 cfs_rq의 leftmost, curr값이 아닌 경우, cfs->rq의 min_vruntime는 이전 값을 유지
	update_min_vruntime(cfs_rq);
	// NO OP
	update_cfs_shares(cfs_rq);
}
...
static inline u64 cfs_rq_clock_task(struct cfs_rq *cfs_rq)
{
	...
	// container_of(cfs_rq, struct rq, cfs) - cfs_rq->throttled_clock_task_time
...
#else /* CONFIG_CFS_BANDWIDTH, not defined */
static inline u64 cfs_rq_clock_task(struct cfs_rq *cfs_rq)
{
...
static void hrtick_update(struct rq *rq)
{
	...
	// 현재 분석 환경에서는 hrtick_enabled()가 false이기 때문에
	// 항상 빠져나옴
...
static void dequeue_task_fair(struct rq *rq, struct task_struct *p, int flags)
{
	...
	for_each_sched_entity(se) {
		...
		*/
		// 현재 분석 버전에서는 cfs_rq_throttled는 항상 0 리턴
		if (cfs_rq_throttled(cfs_rq))
			break;
		cfs_rq->h_nr_running--;
	...
	// cfs_rq->load.weight > 0이였으면 se가 parent_entity()로 변경된다.
	// (현재 분석 버전에서는 null로 세팅)
	// (즉 아래 루프는 타지 않는다.)
	for_each_sched_entity(se) {
		cfs_rq = cfs_rq_of(se);
		cfs_rq->h_nr_running--;
	...
	// 위 루프 구문을 볼 때 cfs_rq_throttled(cfs_rq)가 실패한 경우만
	// 아래 구문을 탈 것이다.
	if (!se) {
		dec_nr_running(rq);
		update_rq_runnable_avg(rq, 1);
...
migrate_task_rq_fair(struct task_struct *p, int next_cpu)
{
	...
	if (se->avg.decay_count) {
		// decay_count > 0인 경우
		// 1) decay_count 재조정.
		// 2) cfs_rq->removed_load += load_avg_contrib
