...
static int dup_mmap(struct mm_struct *mm, struct mm_struct *oldmm)
{
	...
	uprobe_start_dup_mmap(); // NOP
	down_write(&oldmm->mmap_sem); // 세마포어 up
	flush_cache_dup_mm(oldmm); // 전체 cache invalidate로 추정
	uprobe_dup_mmap(oldmm, mm); // NOP 
	/*
	 * Not linked in yet - no deadlock potential:
	 */
	down_write_nested(&mm->mmap_sem, SINGLE_DEPTH_NESTING); // 새로 할당된 mm에 대해 세마포어 up

	mm->locked_vm = 0;
	mm->mmap = NULL;
	mm->mmap_cache = NULL;
	mm->map_count = 0;
	cpumask_clear(mm_cpumask(mm)); // 새로 할당한 mm의 cpu_mask 클리어
	mm->mm_rb = RB_ROOT;
	rb_link = &mm->mm_rb.rb_node;
	rb_parent = NULL;
	pprev = &mm->mmap;
	retval = ksm_fork(mm, oldmm); // NOP, 0 반환
	if (retval)
		goto out;
	retval = khugepaged_fork(mm, oldmm); // NOP, 0반환
	if (retval)
		goto out;
	...
	for (mpnt = oldmm->mmap; mpnt; mpnt = mpnt->vm_next) {
		...
		if (mpnt->vm_flags & VM_DONTCOPY) {
			// mpnt 복사를 하지 않는 경우, mm에서도 페이지 개수를 감소시킨다.
			vm_stat_account(mm, mpnt->vm_flags, mpnt->vm_file,
							-vma_pages(mpnt));
			// 복사를 하지 않기 때문에 다음 vm에 대해 검사한다.
			continue;
		}
		charge = 0;
		if (mpnt->vm_flags & VM_ACCOUNT) {
			...
			if (security_vm_enough_memory_mm(oldmm, len)) /* sic */ // 메모리가 충분한지 확인
				goto fail_nomem;
			charge = len;
		}
		...
		*tmp = *mpnt;
		...
		// 2017-08-19
		retval = vma_dup_policy(mpnt, tmp); // NOP, 0 return
		if (retval)
			goto fail_nomem_policy;
		// mm은 새로 할당한 구조체이기 때문에, 앝은 복사를 해도 무방
		tmp->vm_mm = mm;
		// 2017-08-19
		...
		file = tmp->vm_file;
		if (file) {
			// file 포인터는 얕은 복사가 일어났기 때문에  부모 vm_area_struct(mpnt)와 정보를 공유한다.
			// 따라서 포인터 내 값 수정(ex 레퍼런스 카운팅)동작이 일어난 경우 부모도 같이 영향을 받는다.
			...
			if (unlikely(tmp->vm_flags & VM_NONLINEAR))
				...
				// 2017-08-19
				// tmp -> mpnt의 rb_tree 노드에 링크
				// mapping->immap rb_tree에 tmp 정보 삽입
		 ...
		 */
		if (is_vm_hugetlb_page(tmp)) // NOP, 0 return
